...
const { API_KEY, URL_RECIPE, URL_COMPLEX, API_KEY2, API_KEY3, API_KEY4, API_KEY5, API_KEY6, API_KEY7 } = process.env;
...
const getRecipesId = async (ID) => {
    ...
    try {
        if (uuidValidate(ID)) {
            ...
            const { data } = await axios.get(`${URL_RECIPE}/${ID}/information?apiKey=${API_KEY5}`)
...
const getRecipesTitle = async (name) => {
    ...
    const response = await axios.get(`${URL_COMPLEX}?apiKey=${API_KEY5}&addRecipeInformation=true&number=100`)
...
const getAllRecipes = async () => {
    ...
    const { data } = await axios.get(`${URL_COMPLEX}?apiKey=${API_KEY5}&addRecipeInformation=true&number=100`)
